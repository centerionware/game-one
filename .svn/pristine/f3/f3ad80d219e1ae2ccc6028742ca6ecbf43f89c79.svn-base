void CState_Testing::mouseMoved( const OIS::MouseEvent  & event)
{
  ...
  */
	...
	if(!FreelookEditMode && !FreelookEditModeFPS)
	{
		...
		camera_circle_position_offset.move(event.state.Z.rel);
		 mCamNode->yaw(Ogre::Degree(-(event.state.Z.rel/60)), Ogre::Node::TS_WORLD);
	}
...
void CState_Testing::mousePressed( const OIS::MouseEvent  & event, OIS::MouseButtonID identifier )
{
  ...
  CEGUI::Window* sheet = CEGUI::System::getSingleton().getGUISheet();
	CEGUI::Point mousePos = CEGUI::MouseCursor::getSingleton().getPosition();
	CEGUI::Window* active_window = sheet->getChildAtPosition(mousePos);
	if(active_window &&  ( active_window->getName() != "TextureAtlas") ) return;
  ...
  switch ( identifier )
	...
	if(FreelookEditMode || FreelookEditModeFPS || (active_window&& active_window->getName() == "TextureAtlas")
		) {
		std::string output;
		...
		*/
		...
		} else if(current_editmode == CircleScraper)
			m_EnterRadius->show();
		...
		else if(current_editmode == ClipboardCopy) {
			NewHeightmapLoader::get_instance()->ClipboardCopy();
		}
