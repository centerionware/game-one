...
void DotSceneLoader::processEntity(TiXmlElement *XMLNode, SceneNode *pParent)
{
	...
	try
	{
		// All should be loaded from the resources.cfg!!
...
std::string DotSceneLoader::getAttrib(TiXmlElement *XMLNode, const String &attrib, const String &defaultValue)
{
	if(XMLNode->Attribute(attrib.c_str()))
		return (string)( XMLNode->Attribute(attrib.c_str()));
	else
		return defaultValue;
}
...
{
	if(XMLNode->Attribute(attrib.c_str())) {
		Ogre::String str = XMLNode->Attribute(attrib.c_str());
		return StringConverter::parseReal( str );
	} else
		return defaultValue;
}
