...
bool CState_MainMenu_Login::OnLogin( const CEGUI::EventArgs &event )
{
	...
	tryingToLogin = true;
	if (this->mPlayer != boost::shared_ptr<inet_player>()) {
		auto i = inet_player::Players.begin(), ie = inet_player::Players.end();
		for(;i!=ie;++i) if((*i) == mPlayer) { inet_player::Players.erase(i); break; }
		mPlayer.reset();
	}
	LoginTimer.reset();
	this->mPlayer.reset(new inet_player());
	inet_player::Players.push_back(mPlayer);
	this->mPlayer->login_menu_conndisconn();
...
void CState_MainMenu_Login::ConnectionSuccess() {
	if(tryingToLogin) { // Login form state
		...
		this->mPlayer->send_text(update, this->mPlayer->socket);
