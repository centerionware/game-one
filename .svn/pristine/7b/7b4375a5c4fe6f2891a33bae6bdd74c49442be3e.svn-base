...
struct threadpool_sendstruct {
  ...
  void main_thread(int whatever) {
    std::cout << "Initializing send thread\n";
    while(threadpool_sendstruct::on){ 
      std::cout << "Waiting for post to semaphore.\n";
      main_t.wait();
...
// 	
    ...
    std::cout << "Send thread terminated.\n";
  }
  ...
  void sudden_close(int socket){ 
      mDat.lock();
      auto i = mData.begin();//, e = mData.end();
      while(i!=mData.end()){ 
	if( (*i).socket == socket) {
	  i = mData.erase(i);
	} else i++;
...
void remove_player(unsigned int socket) {
	...
	noblock_sendtext.sudden_close(socket);
	...
	for(;iter!=end;iter++) 
		...
		if(chunk_quick_cache::ptr) chunk_quick_cache::ptr->remove_player((*iter).get());
		inet_player::Players.erase(iter); return; 
	}
