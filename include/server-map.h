...
struct basic_chunk {
	basic_chunk() : id(-1), x(0),y(0),z(0){}
	basic_chunk(const basic_chunk &other):id(other.id),x(other.x),y(other.y),z(other.z) {
		std::copy(other.voxels.begin(), other.voxels.end(), std::back_inserter(voxels));
		std::copy(other.matids.begin(), other.matids.end(), std::back_inserter(matids));
	}
	unsigned int id;
	float x,y,z;
...
struct basic_chunk_update {
  ...
  basic_chunk_update() { chunk.id = -1; timestamp = 0; }
...
public:
	Chunk * getSelChunk() { return selectedChunk; }
	unsigned short getSelVoxel() { return selectedVoxel; }
	...
	std::vector<std::string> editor_circle_scraper_gen1(vec3 pnt, int radius);
...
struct chunk_lock {
	...
	static void clear_locks() {
		for(auto i = chunk_locks.begin(); i!=chunk_locks.end();i=chunk_locks.erase(i)) {
			(*i).second.first->lock();
		}
	}
	Chunk *c;
	...
	~chunk_lock() { unlock(); }
	void lock() { auto &a = (*(chunk_locks.find(c))).second; a.first->lock(); a.second+=1; }
...
public:
	...
	std::vector<unsigned short> madelights;
