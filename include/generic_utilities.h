   ...
   //   boost::shared_ptr<vec4_vec3_vector> tt; // to reduce (de)constructor calls for std::vector when it's not even being used...
	...
	return (x >=a && x <= b && y >=a && y <= b && z >=a && z <= b);
      ...
      inline GOREAL length() {
		  return Ogre::Math::Sqrt(dot_product(*this));// (x*x) + (y*y) + (z*z) );
      }
};
struct wvec3 : private vec3 { // wild vec3. woo!. trust me.
	  inline bool operator==(const vec3 &other) const {
		  bool a=false, b=false, c=false;
		  if(x==0 &&other.x !=0) return false;
		  if(y==0 &&other.y !=0) return false;
		  if(z==0 &&other.z !=0) return false;
		  a = other.x ==0?true:other.x==x;
		  b = other.y ==0?true:other.y==y;
		  c = other.z ==0?true:other.z==z;
			return a&&b&&c;
		  return x==0?true:(other.x==0?true:x==other.x) && y==0?true:(other.y==0?true:y==other.y) && z==0?true:(other.z==0?true:z==other.z);
      }
	  template<class T, class U, class V>wvec3(T _x, U _y, V _z) {
		  memset(this, 0, sizeof(*this));
		  x=_x, y=_y, z=_z;
	  }
};
...
boost::shared_array<vec3*> arr_93_mul(vec3 *array, T amt) {
	vec3 *ptr = new vec3[9*3];
	boost::shared_array<vec3*> threadsafe_result_container(&ptr);
