...
struct insert_sorted_chunks { 
	...
	void insert(Chunk * ele, const vec3 &oc){//Ogre::Camera *mCam) {
		...
		else if((*z)->id == ele->id && ele->id!=(unsigned int)(-1)) { 
		std::stringstream output;
		output << "Trying to insert duplicate chunk into memory with id of " << ele->id << "\n";
		output << "The positions of the elements are: " << (*z)->position.print("z") << " " << ele->position.print("ele")<<std::endl;
		mut_array[0].unlock(); throw output.str().c_str(); }
