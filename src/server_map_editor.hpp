...
void NewHeightmapLoader::ClipboardCopy() {
// first, get the selected chunk and voxel;
		vec3 temp_v = selectedChunk->position+(from_voxid(selectedVoxel)-vec3(15,30,15)); 
		bool bits[27];
		...
		bool edge_list[12] = {false,false,false,false,false,false,false,false,false,false,false,false}; 
		wvec3 edge_list_neighbors[] = { 
			wvec3(1,1,0), wvec3(-1,1,0), wvec3(0,1,1),wvec3(0,1,-1),
			wvec3(1,-1,0), wvec3(-1,-1,0), wvec3(0,-1,1),wvec3(0,-1,-1),
			wvec3(1,0,1), wvec3(-1,0,1), wvec3(-1,0,1),wvec3(1,0,-1)
		};
			...
			if(ctr != 13) {
				...
				if(bc != NULL) {
					...
					if(bc->voxel_exists(to_voxid(varp))) {
						for(int lctr = 0; lctr<12; lctr++) {
							if( edge_list_neighbors[lctr] == neighbor_list[ctr] ) {
								edge_list[lctr] = true;
							}
						}
						bits[ctr] = true;
		...
		int cubeindex = 0, cubeindextwo = 0;
		...
		stemplate.append("\t\tif(lpacked == ");
		stemplate.append(to_string(cubeindex));
		stemplate.append(" && lpackedtwo == ");
		stemplate.append(to_string(cubeindextwo));
		stemplate.append(") { } ");
		// now stemplate should contain a valid glsl if() statement
		// that can be used to manipulate the way whatever configuration is selected is in.
#ifdef WIN32
		const char* output = stemplate.c_str();
...
#include "server_map_circlescraper_gen.hpp"