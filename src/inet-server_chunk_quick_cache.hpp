...
//		int ctr = 0;
		for(;iter!=end;++iter) {
			if(get_distance( (*iter).position, in->position) < 675 && (*iter).position.y-in->position.y <75&&(*iter).position.y-in->position.y >-75) {
...
//				ctr++;
		...
		for(int x = 0; x < 9; ++x) {
		      vec3 pv = pi_map[x];
		      pv.x = pv.x *31;
		      pv.y = pv.y=positioning.y;
		      pv.z = pv.z *31;
		      vec3 mval = (pv)+positioning;
		      if(!exists( mval )) {
			//  locktex.unlock();
...
//		chunk_quick_cache::issetup = true;
	...
	void reset(unsigned int chunk_id) {
		for(auto i = chnks.begin(), e = chnks.end(); i!=e; ++i) if((*i).id == chunk_id) { chnks.erase(i); break; }
		std::stringstream query;
		query << "select X,Y,Z,id, UNIX_TIMESTAMP((select max(lastupdate) from Chunk_Voxels where chunk_id = C.id)), UNIX_TIMESTAMP((select max(lastupdate) from RemovedVoxels where chunk_id = C.id)) from Chunks as C where id='?'";
		MySQL_Helper::Query qry(database_connection);
		qry.prepare(query.str(), chunk_id);
		qry.run();
		size_t numrows = qry.numrows;
		if(numrows != 1) { std::cout << "Requested " << chunk_id << " And got multiple rows\n"; return; }
	    
	    	MYSQL_ROW& mrow = qry.row();
		unsigned long *lengths = qry.lengths();
		aChunk c;
		c.position.x = fromstring<float>(mrow[0],lengths[0]);
		c.position.y = fromstring<float>(mrow[1],lengths[1]);
		c.position.z = fromstring<float>(mrow[2],lengths[2]);
	...
	bool exists(vec3 rc) {
	 // mut_c_locker locktex();
	  ...
	  i = genned_chunks.begin(), ie = genned_chunks.end();
	  for(;i!=ie;++i) if((*i).position==rc) return true;
