struct ClientNetmodule : public net_module {
    void initialize() {
	mPlayer.lock()->netbind(mPlayer.lock().get(), &inet_player::getPlayerList, OLC, GetPlayerList);
	mPlayer.lock()->netbind(mPlayer.lock().get(), &inet_player::modifyPlayer, OLC, ModifyPlayer);
	mPlayer.lock()->netbind(mPlayer.lock().get(), &inet_player::deletePlayer, OLC, DeletePlayer);
	mPlayer.lock()->netbind(mPlayer.lock().get(), &inet_player::lockPlayer, OLC, LockPlayer);
	mPlayer.lock()->netbind(mPlayer.lock().get(), &inet_player::getPlayerDetails, OLC, GetPlayerDetails);
    }
};
ClientNetmodule sleepytimetea;
